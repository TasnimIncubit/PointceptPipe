...
class SemSegTester(object):
    ...
    def __call__(self, cfg, test_loader, model):
        ...
        save_path = os.path.join(cfg.save_path, "result", "single_100")
        make_dirs(save_path)
        ...
        for i, input_dict in enumerate(test_loader):
            ...
            # print('input_dict size in test:', len(input_dict))
            # print("input_dict keys in test:", input_dict[0].keys())
            input_dict = input_dict[0]
            print("test input_dict keys in eval:", input_dict.keys())
            print("test input_dict coord size: ", input_dict["coord"].size())
            print("test input_dict discrete coord size: ", input_dict["discrete_coord"].size())
            print("test input_dict segment size: ", input_dict["segment"].size())
            for key in input_dict.keys():
                if isinstance(input_dict[key], torch.Tensor):
                    input_dict[key] = input_dict[key].cuda(non_blocking=True)
            with torch.no_grad():
                output_dict = model(input_dict)
            print(" test output_dict keys in eval:", output_dict.keys())
            
            output = output_dict["seg_logits"]

            print("test output_dict seg_logits size: ", output.size())
            
            count += 1
            np_surface = input_dict["coord"].cpu().detach().numpy()
            np_center = output_dict["seg_logits"].cpu().detach().numpy()
            np_all_points = np.stack((np_surface, np_center), axis = 1)
            numpy_filename = 'output_' + format(int(count), '05d') + '.npy'
            np.save(os.path.join(save_path, numpy_filename), np_all_points) # sget rotated values
