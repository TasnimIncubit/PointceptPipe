...
class SemSegEvaluator(HookBase):
    ...
    def eval(self):
        ...
        self.trainer.model.eval()
        for i, input_dict in enumerate(self.trainer.val_loader):
            print("input_dict keys in eval:", input_dict.keys())
            print("input_dict coord size: ", input_dict["coord"].size())
            print("input_dict discrete coord size: ", input_dict["discrete_coord"].size())
            print("input_dict segment size: ", input_dict["segment"].size())
            for key in input_dict.keys():
                if isinstance(input_dict[key], torch.Tensor):
                    input_dict[key] = input_dict[key].cuda(non_blocking=True)
            with torch.no_grad():
                output_dict = self.trainer.model(input_dict)
            print("output_dict keys in eval:", output_dict.keys())
            
            output = output_dict["seg_logits"]

            print("output_dict seg_logits size: ", output.size())
            loss = output_dict["loss"]
            pred = output.max(1)[1]
        ...
        self.trainer.comm_info["current_metric_name"] = "loss_avg"
    ...
    def after_train(self):
        ...
        # self.trainer.logger.info("Training done gg")
        
        self.trainer.logger.info("Best {}: {:.4f}".format(
            "loss_avg", self.trainer.best_metric_value))
